...
\subsection{Sparse Convergence}
\begin{itemize}
	\item Do nothing, nodes will eventually converge
	\item If too sparse, will not converge
	\item Requires no extra data transmission
\end{itemize}

\subsection{Sparse Relay}
\begin{itemize}
	\item When recv a model update, check if it is newer than what we already have and is not from us, if it is, ping it to all neighbours
	\item The whole network acts as if it is dense
	\item Requires extra transmission
\end{itemize}